...
- [Firecrawl CLI](https://www.firecrawl.dev) — движок скрейпинга и поиска
- [Tavily CLI](https://docs.tavily.com/documentation/tavily-cli) — search + research API (бесплатный тариф OK)

**Опционально (graceful degradation если отсутствует):**

- [Exa MCP](https://exa.ai) — neural semantic search (L2+, v0.5.0+)
- [OpenAI Codex CLI](https://developers.openai.com/codex/cli) + подписка ChatGPT Pro — cross-model канал (L2+, v0.2+)

> **v0.9.0 breaking change.** Зависимость от Perplexity MCP удалена. Perplexity Answer Channel (L0/L2/L3/L5) заменён на Tavily Research с `--output-schema` для структурированных verdict'ов в fact-check. См. [CHANGELOG.md](CHANGELOG.md).
...
- [x] v0.6 — Perplexity MCP (answer engine) · _удалено в v0.9.0_
- [x] v0.7 — Авто-синк L5 vault'ов в Obsidian
- [x] v0.8 — `/osint-research` skill (passive entity recon)
- [x] v0.9 — **Удалена зависимость от Perplexity.** L0 → Fireplexity-pattern (FC search-with-scrape + AI summary). L2/L3/L5 answer/fact-check каналы → Tavily Research с `--output-schema` для структурированных verdict'ов. L3 fact-check теперь schema-based (verdict + evidence_urls + confidence + rationale). FC × Tavily diversity на каждом search-шаге.
- [ ] v1.0 — Multi-LLM triangulation (Codex + Kimi + Claude consensus voting на L3+)
